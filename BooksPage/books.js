    ...
    .then(data => {
        ...
        newBookList.map((book, i) => {
            let fixedSaleability =  book.saleInfo.saleability.replace(/_/g, ' ')
                ...
                if(!book.volumeInfo.description) book.volumeInfo.description = 'This book has no discription'
                if(!book.volumeInfo.categories) book.volumeInfo.categories = 'unknown'
                if(!book.volumeInfo.publisher) book.volumeInfo.publisher = 'Unknown publisher'
                !book.saleInfo.listPrice ? Book.price = fixedSaleability 
                : book.saleInfo.listPrice.amount === 0 ? Book.price = "FREE" 
